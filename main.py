...
def exibir_extrato(banco: BancoController):
    ...
    if opcao == "1":
        tipo = "DespositoController"
    elif opcao == "2":
        tipo = "SaqueController"
...
def main():
    ...
    while True:
        ...
        mostrar_menu_movimentacao_conta(
            banco.cliente_logado.conta.saldo, banco.id_cliente_logado
        )
        entrada = str(input("Digite sua escolha: "))

        if entrada == "1":
            transacao(banco, DespositoController)
        if entrada == "2":
            transacao(banco, SaqueController)
        if entrada == "3":
            exibir_extrato(banco)
        if entrada == "4":
            banco.cliente_logado.listar_conta()
        if entrada == "q":
